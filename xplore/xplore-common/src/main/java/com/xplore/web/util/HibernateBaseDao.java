...
public abstract class HibernateBaseDao<T, PK extends Serializable> {
    ...
    public List<T> findIn(final String hql, final String placeHolder, final List values) {
    ...
    public Query createQueryIn(final String queryString, final String placeHolder, final List values) {
        Assert.hasText(queryString);
        Query query = getSession().createQuery(queryString);
        if (values != null) {
     ...
     */
    public Page findBySql(final Page<T> page, final String sql,
                             final Object[] values) {
    ...
    public Page findBySql(final Page<T> page, final Class clazz, final String sql,
                          final Object[] values) {
        if (page.getFirst() >= 0 && page.getPageSize() > 0 && sql != null) {
            List<T> list = hibernateTemplate.executeFind(
                    new HibernateCallback<Object>() {
